...
`packages/coding-agent/src/extensibility/hooks/index.ts` and the package subpath `/pi-coding-agent/extensibility/hooks` export:
...
The package root (`/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
