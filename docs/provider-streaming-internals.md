...
This document explains how token/tool streaming is normalized in `/pi-ai`, then propagated through `/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `/pi-ai`
