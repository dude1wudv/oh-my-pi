...
- `packages/coding-agent/src/session/auth-storage.ts` — re-exports `AuthStorage` from `/pi-ai`; API key + OAuth resolution order
...
1. Load built-in providers/models from `/pi-catalog` (`getBundledProviders` / `getBundledModels`).
