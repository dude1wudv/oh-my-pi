...
This is the contributor path for moving a measured JS/TS hot path into `crates/pi-natives` and exposing it through `/pi-natives`.
...
- Import the generated root symbol or intentional lazy subpath from `/pi-natives`.
