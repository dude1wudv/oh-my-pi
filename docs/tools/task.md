...
- Key collaborators:
  ...
  - `/pi-natives` (`crates/pi-iso`) — isolation PAL: `isoResolve` / `isoStart` / `isoStop` backend resolution and fallback.
...
- Validation ownership: ordinary task workers MUST NOT run project-wide or large-scale test/validation gates; they MAY run only targeted checks directly required by their assigned contract. Review orchestration keeps parallel reviewers focused and assigns exactly one `FINAL VALIDATION` reviewer to run the canonical repository-wide suite once after review work settles.
