...
The SDK is the in-process integration surface for `/pi-coding-agent`.
...
The package root, `/pi-coding-agent`, is the complete embedding surface. It includes `createAgentSession` and the focused `/sdk` exports, plus lower-level session, auth, model, mode, extension, and tool APIs.
...
```ts
import { createAgentSession, SessionManager } from "/pi-coding-agent";
...
```ts
import type { AgentSession } from "/pi-coding-agent";
