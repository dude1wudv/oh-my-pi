...
- optional `tools`, `spawns`, prioritized `model` list, `thinkingLevel`, `output`, `blocking`, `autoloadSkills`, `readSummarize`, `prewalk`, `advisor`, `restrictTools`
...
- `restrict-tools: true` restricts the child session to its declared `tools` plus required `yield`, disables ambient MCP, extensions, custom tools, LSP, and IRC, and persists `restrictToolNames` for cold revival. It does not sandbox `bash`; the agent prompt must still limit commands to the approved local scope.
