...
- Package: `/omp-stats`; bin: `omp-stats`; main user path: `omp stats`.
...
- Package: public `/omptype`; install with `bun add /omptype`; requires Bun 1.3.14 or newer.
...
- Public surfaces: `/omptype` for native authoring, `/typebox` and `/zod` for compatibility builders, and `/ark` for the alias-free ArkType compatibility facade.
...
- Package: private `/typescript-edit-benchmark`; support library with no standalone bin.
...
- Package: private `/pi-metaharness`; bin: `metaharness`.
...
- Package: private `/browser-relay`; user command: `omp browser-relay`.
...
- Package: private `/collab-web`; production client: <https://my.omp.sh/>.
...
- Package: public `/snapcompact`; install with `bun add /snapcompact`; requires
  Bun 1.3.14 or newer.
...
- Runtime constraint: rasterization and PNG encoding require `/pi-natives`.
...
- Package: public `/pi-mnemopi`; bin: `mnemopi`; requires Bun 1.3.14 or newer. Install globally with `bun add --global /pi-mnemopi`, then run `mnemopi <command>`. From a source checkout, `bun packages/mnemopi/src/cli.ts <command>` runs the same entrypoint.
