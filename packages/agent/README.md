...
Stateful agent with tool execution and event streaming. Built on `@dude1wudv/pi-ai`.

## Installation

```bash
npm install @dude1wudv/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@dude1wudv/pi-agent";
import { getModel } from "@dude1wudv/pi-ai";
...
```typescript
declare module "@dude1wudv/pi-agent" {
...
```typescript
import { Agent, streamProxy } from "@dude1wudv/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@dude1wudv/pi-agent";
