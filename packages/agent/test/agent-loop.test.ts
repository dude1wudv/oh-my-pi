...
} from "@dude1wudv/pi-agent-core/agent-loop";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
...
import { ASIDE_MESSAGE_COMMIT, ASIDE_MESSAGE_DISCARD } from "@dude1wudv/pi-agent-core/types";
import type { AssistantMessage, AssistantMessageEvent, Context, Message, ToolResultMessage } from "@dude1wudv/pi-ai";
import { createMockModel, type MockResponse } from "@dude1wudv/pi-ai/providers/mock";
...
declare module "@dude1wudv/pi-agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@dude1wudv/pi-ai/utils/block-symbols");
