...
/**
 * Native addon loader for `@dude1wudv/pi-natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@dude1wudv/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * `node_modules/@dude1wudv/pi-natives/native/`, leaving an old binary next to a
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	if (residentSentinel && diskHasExpectedSentinel) {
		...
		throw new Error(
			`Loaded ${candidate}, which exposes the @dude1wudv/pi-natives@${residentVersion} version ` +
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @dude1wudv/pi-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @dude1wudv/pi-natives\n" +
