...
import { $which, getPuppeteerDir, logger, removeWithRetries } from "@dude1wudv/pi-utils";
import type * as BrowsersNs from "@dude1wudv/pi-utils/browsers";
...
async function loadBrowsers(): Promise<typeof BrowsersNs> {
	if (!browsersModule) {
		browsersModule = await import("@dude1wudv/pi-utils/browsers");
	}
