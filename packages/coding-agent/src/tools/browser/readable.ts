import type * as DomNs from "@dude1wudv/pi-utils/dom";
import type * as ReadabilityNs from "@dude1wudv/pi-utils/readability";
...
async function loadReadability(): Promise<typeof ReadabilityNs> {
	if (!readabilityModule) {
		readabilityModule = await import("@dude1wudv/pi-utils/readability");
	}
...
async function loadDom(): Promise<typeof DomNs> {
	if (!domModule) {
		domModule = await import("@dude1wudv/pi-utils/dom");
	}
	return domModule;
}
