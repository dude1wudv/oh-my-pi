...
<batch-gate>
When an async task batch is registered, runtime owns a Main `AsyncBatchGate`. Enter passive `WAIT_ALL`: individual completions update gate counters only and NEVER trigger a fresh Main follow-up, repeated `hub wait`, or coordination nudge. The configured `async.batchWakeInterval` defaults to `off`; off disables timer wakes. A batch emits one aggregate `async-batch-result` on the first child failure or after every child settles. Timer wakes, when explicitly enabled, are also aggregate. User messages, cancellation, explicit failure/blocker, and safety events may wake immediately, but generation tokens prevent duplicate deliveries. Close the gate after terminal cleanup without replaying individual child deliveries.
</batch-gate>
<rules>
