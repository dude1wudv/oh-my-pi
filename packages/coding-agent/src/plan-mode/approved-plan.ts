...
export async function exportApprovedProjectPlan(input: {
	cwd: string;
	planContent: string;
	title: string;
	createdDate?: string;
}): Promise<{ projectPlanPath: string; absolutePath: string; planId: string }> {
	const root = path.resolve(input.cwd);
	const plansRoot = path.resolve(root, ".omp", "plans");
	await fs.mkdir(plansRoot, { recursive: true });
	const normalized = normalizePlanTitle(input.title).title;
	const planId =
		normalized
			.replace(/_/g, "-")
			.replace(/-plan$/i, "")
	...
	const date = input.createdDate ?? localDate(new Date());
	for (let suffix = 1; ; suffix += 1) {
		const name = `${date}-${planId}${suffix === 1 ? "" : `-${suffix}`}.md`;
		if (!PROJECT_PLAN_FILENAME_RE.test(name)) throw new Error(`Invalid project plan filename: ${name}`);
		const absolutePath = resolveProjectPlanPath(root, path.posix.join(".omp/plans", name));
		const projectPlanPath = projectPlanRelativePath(root, absolutePath);
		try {
			await fs.writeFile(
				absolutePath,
				initialProjectMetadata(input.planContent, normalized, planId, projectPlanPath, new Date()),
				{
					encoding: "utf8",
					flag: "wx",
				},
			);
			return { projectPlanPath, absolutePath, planId };
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code === "EEXIST") continue;
			throw error;
		}
