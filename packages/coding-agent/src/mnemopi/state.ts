...
import type * as MnemopiNs from "@dude1wudv/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@dude1wudv/pi-mnemopi";
import type * as MnemopiCoreNs from "@dude1wudv/pi-mnemopi/core";
import type { LocalModelInitializer } from "@dude1wudv/pi-mnemopi/core";
import { logger, toError } from "@dude1wudv/pi-utils";
import {
	composeRecallQuery,
	formatCurrentTime,
...
/**
 * Lazily load `/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@dude1wudv/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@dude1wudv/pi-mnemopi/core");
