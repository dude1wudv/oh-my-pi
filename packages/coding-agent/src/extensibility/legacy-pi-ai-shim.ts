 ...
 * `/pi-ai` (or one of its aliased scopes like `@earendil-works/pi-ai`
 ...
 * bare package root. Subpath imports (`/pi-ai/oauth`, etc.)
 ...
 * `/pi-catalog/models`. `getModel`/`getModels` are the historical
 ...
 * should wrap with `retryTransientCompletion` from `@dude1wudv/pi-ai`.
 */
 ...
 * `/pi-ai` barrel no longer forwards. Each symbol still exists in the
 ...
 * `/pi-ai/error`) and the JSON-repair helpers that omp relocated to
 * `/pi-utils`. Upstream root symbols with no omp equivalent are
 * intentionally not shimmed — the package has diverged and there is nothing to
 * forward.
 */
export { isContextOverflow } from "@dude1wudv/pi-ai/error";
export { parseJsonWithRepair, parseStreamingJson, repairJson } from "@dude1wudv/pi-utils";
