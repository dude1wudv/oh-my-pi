 ...
 * `/pi-coding-agent` (or one of its aliased scopes like
 ...
 * the same module identity as a direct `/pi-coding-agent` import.
 */

import { Database } from "bun:sqlite";
import * as fs from "node:fs";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@dude1wudv/pi-agent-core";
import { type AuthCredential, SqliteAuthCredentialStore, type TSchema } from "@dude1wudv/pi-ai";
import { piEscapeRegexLiteral, piJoinPath } from "@dude1wudv/pi-ai/providers/cursor-pi-args";
import { getKeybindings, type Keybinding, Text } from "@dude1wudv/pi-tui";
import {
	getAgentDbPath,
	getAgentDir,
	getProjectDir,
...
// `getAgentDir`; `getProjectDir` (a `/pi-utils` helper) and
...
export { getProjectDir } from "@dude1wudv/pi-utils";
...
// `/pi-agent-core/compaction`, and the coding-agent barrel below does
...
export { compact, estimateTokens, serializeConversation } from "@dude1wudv/pi-agent-core/compaction";
...
// `CONFIG_DIR_NAME` lives in `/pi-utils` and `parseArgs` in
...
export { CONFIG_DIR_NAME } from "@dude1wudv/pi-utils";
