...
import type { CliConfig, CommandMetadata } from "@dude1wudv/pi-utils/cli";
import {
	APP_NAME,
	getActiveProfile,
	MIN_BUN_VERSION,
	...
	setProfile,
	VERSION,
} from "@dude1wudv/pi-utils/dirs";
import { interceptUnhandledRejections } from "@dude1wudv/pi-utils/postmortem";
import { setProcessName } from "@dude1wudv/pi-utils/process-name";
import { declareWorkerHostEntry, installWorkerInbox, isWorkerHostSelector } from "@dude1wudv/pi-utils/worker-host";
...
// `/pi-utils/env` eagerly loads `.env` from the agent directory at
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@dude1wudv/omp-stats");
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@dude1wudv/omp-stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	// `/pi-utils/env` here would snapshot the wrong agent `.env`.
