 ...
 *
 * oh-my-pi's agent core (`/pi-agent-core`) emits OpenTelemetry GenAI
...
import { logger, postmortem } from "@dude1wudv/pi-utils";
import {
	type Attributes,
	type AttributeValue,
	type Counter,
...
async function registerProviders(signalConfig: SignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("/pi-coding-agent"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("/pi-coding-agent");
