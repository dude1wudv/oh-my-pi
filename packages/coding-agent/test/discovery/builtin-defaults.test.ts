...
import {
	...
	type Rule,
	ruleCapability,
} from "@dude1wudv/pi-coding-agent/capability/rule";
...
import { TtsrManager, type TtsrMatchContext } from "@dude1wudv/pi-coding-agent/export/ttsr";
...
describe("builtin-defaults rule provider", () => {
	...
	it("fires ts-no-local-is-record on local function and lambda definitions", async () => {
		...
		manager.resetBuffer();
		expect(
			manager.checkDelta('import { isRecord } from "@dude1wudv/pi-utils";', {
				source: "tool",
				toolName: "write",
				filePaths: ["packages/x/src/foo.ts"],
			}),
