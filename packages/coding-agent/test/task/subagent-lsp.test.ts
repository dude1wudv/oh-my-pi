...
import type { AgentDefinition, TaskParams } from "@dude1wudv/pi-coding-agent/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@dude1wudv/pi-coding-agent/task/worktree";
...
import "@dude1wudv/pi-coding-agent/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("clamps plan-mode mixed-capability tools despite ordinary settings", async () => {
		...
		const { getOptions } = mockCreateAgentSession();
		const planMode = { enabled: true, planFilePath: ".omp/plans/2026-08-18-plan.md" };
