...
import { Tokenizer } from "@dude1wudv/pi-agent-core";
import type {
...
import type { SessionManager } from "@dude1wudv/pi-coding-agent/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "@dude1wudv/pi-coding-agent/slash-commands/acp-builtins";
import { removeWithRetries, setProjectDir } from "@dude1wudv/pi-utils";
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		...
		const mcpModule = await import("@dude1wudv/pi-coding-agent/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@dude1wudv/pi-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@dude1wudv/pi-coding-agent/extensibility/plugins/marketplace");
