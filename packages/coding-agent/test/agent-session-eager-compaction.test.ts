...
import type { Model, TextContent } from "@dude1wudv/pi-ai";
import * as codexResponses from "@dude1wudv/pi-ai/providers/openai-codex-responses";
...
import { TodoTool, type ToolSession, USER_TODO_EDIT_CUSTOM_TYPE } from "@dude1wudv/pi-coding-agent/tools";
...
describe("AgentSession eager prelude re-injection after compaction", () => {
	...
	it("does not re-inject the eager task reminder in plan mode", async () => {
		const { session, waitForCall } = await createHarness();
		session.setPlanModeState({ enabled: true, planFilePath: ".omp/plans/2026-08-18-plan.md" });
		stubCompaction();
