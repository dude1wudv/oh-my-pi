...
import { createWorkerSubprocess, type SpawnedSubprocess } from "@dude1wudv/pi-coding-agent/subprocess/worker-client";
...
describe("issue #4324 — worker subprocess stderr survives to the exit error", () => {
	...
	it("does not keep the parent alive while an unref'd worker stays idle", async () => {
		...
		const wrapperScript = `
			const { createWorkerSubprocess } = await import("@dude1wudv/pi-coding-agent/subprocess/worker-client");
