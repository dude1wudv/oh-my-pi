...
import { __buildLegacyPiPackageRootOverrides } from "@dude1wudv/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
`,
	...
	it("serves @dude1wudv/pi-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@dude1wudv/pi-ai/oauth"]).toBe("omp-legacy-pi-bundled:@dude1wudv/pi-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@dude1wudv/pi-ai/oauth/anthropic"]).toBe(
			"omp-legacy-pi-bundled:@dude1wudv/pi-ai/oauth/anthropic",
		);
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@dude1wudv/pi-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@dude1wudv/pi-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@dude1wudv/pi-ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@dude1wudv/pi-ai/providers/cursor-pi-args";
...
`,
	...
	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@dude1wudv/pi-coding-agent/web/search/providers/xai",
			"@dude1wudv/pi-coding-agent/web/search/providers/tinyfish",
			"@dude1wudv/pi-coding-agent/web/search/providers/firecrawl",
			"@dude1wudv/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@dude1wudv/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@dude1wudv/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@dude1wudv/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@dude1wudv/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			...
			if (
				key === "@dude1wudv/pi-ai" ||
				key === "@dude1wudv/pi-coding-agent" ||
				key === "@dude1wudv/pi-tui" ||
				key === "typebox"
			)
				continue;
	...
	it("keeps pi-ai/pi-coding-agent/pi-tui roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@dude1wudv/pi-ai"]).toBeDefined();
		expect(overrides["@dude1wudv/pi-ai"]).not.toBe("omp-legacy-pi-bundled:@dude1wudv/pi-ai/oauth");
		expect(overrides["@dude1wudv/pi-coding-agent"]).toBeDefined();
		expect(overrides["@dude1wudv/pi-tui"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@dude1wudv/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@dude1wudv/pi-coding-agent/tools");
	...
	it("bundles nested wildcard subpaths so a compiled extension can import them", () => {
		...
		expect(bundledModuleKeys.has("@dude1wudv/pi-coding-agent/slash-commands/helpers/active-oauth-account")).toBe(true);
		// Directory index modules stay excluded: `./x/*` must not serve `x/y`
		// from `y/index.ts`, which Node would not resolve either.
		expect(bundledModuleKeys.has("@dude1wudv/pi-coding-agent/modes/theme/defaults/index")).toBe(false);
	});
});
