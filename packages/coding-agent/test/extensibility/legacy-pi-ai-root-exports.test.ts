...
import type { AssistantMessage, FetchImpl } from "@dude1wudv/pi-ai";
...
} from "@dude1wudv/pi-coding-agent/extensibility/legacy-pi-ai-shim";

// Issue #6859: pi extensions import runtime helpers from the `@earendil-works/pi-ai`
// (aliased to `@dude1wudv/pi-ai`) package root that omp's barrel no longer forwards.
// `isContextOverflow` moved under `@dude1wudv/pi-ai/error` and the JSON-repair
// helpers moved to `@dude1wudv/pi-utils`, so `export * from "@dude1wudv/pi-ai"` left
