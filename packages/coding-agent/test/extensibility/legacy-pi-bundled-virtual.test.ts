...
// `@dude1wudv/pi-*` imports through virtual modules backed by live host module
...
describe("legacy-pi bundled virtual module synthesizer (issue #3423)", () => {
	...
	it("emits one ES named export per enumerable namespace key", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@dude1wudv/pi-coding-agent", modules);
		expect(src).toContain(
			`const __omp_bundled = globalThis[${JSON.stringify(globalKey)}]["@dude1wudv/pi-coding-agent"];`,
		);
	...
	it("forwards `default` through `export default` so default imports survive", () => {
		const src = __synthesizeLegacyPiBundledSourceWithModules("@dude1wudv/pi-utils", modules);
	...
	it("throws when asked to synthesize a key the bundled modules do not cover", () => {
		expect(() => __synthesizeLegacyPiBundledSourceWithModules("@dude1wudv/pi-not-bundled", modules)).toThrow(
	...
	it("routes Bun plugin resolution through the bundled namespace so onLoad can serve extension imports", async () => {
		...
		await Bun.write(
			entryPath,
			['export { legacyAnswer } from "omp-legacy-pi-bundled:@dude1wudv/pi-utils";', ""].join("\n"),
		);

		expect(resolveBundledVirtualSpecifier("@dude1wudv/pi-utils")).toEqual({
			namespace: "omp-legacy-pi-bundled",
			path: "@dude1wudv/pi-utils",
		});
		expect(resolveBundledVirtualSpecifier("omp-legacy-pi-bundled:@dude1wudv/pi-utils")).toEqual({
			namespace: "omp-legacy-pi-bundled",
			path: "@dude1wudv/pi-utils",
		});
		...
		expect(onLoadPaths).toEqual(["@dude1wudv/pi-utils"]);
		...
		expect(bundledModule.legacyAnswer).toBe("served:@dude1wudv/pi-utils");
	});
});
