...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@dude1wudv/pi-ai": "/tmp/exists-ai.js",
			"@dude1wudv/pi-utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@dude1wudv/pi-ai": "/tmp/exists-ai.js",
			"@dude1wudv/pi-coding-agent": "/tmp/exists-shim.js",
			"@dude1wudv/pi-utils": "/$bunfs/root/packages/utils/src/index.js",
			"@dude1wudv/pi-tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@dude1wudv/pi-utils");
		expect(result).not.toHaveProperty("@dude1wudv/pi-tui");
	});
	...
	it("keeps virtual omp-legacy-pi-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@dude1wudv/pi-ai": "omp-legacy-pi-bundled:@dude1wudv/pi-ai",
			"@dude1wudv/pi-coding-agent": "omp-legacy-pi-bundled:@dude1wudv/pi-coding-agent",
			"@dude1wudv/pi-agent-core": "omp-legacy-pi-bundled:@dude1wudv/pi-agent-core",
			"@dude1wudv/pi-natives": "omp-legacy-pi-bundled:@dude1wudv/pi-natives",
			"@dude1wudv/pi-tui": "omp-legacy-pi-bundled:@dude1wudv/pi-tui",
			"@dude1wudv/pi-utils": "omp-legacy-pi-bundled:@dude1wudv/pi-utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@dude1wudv/pi-ai": "omp-legacy-pi-bundled:@dude1wudv/pi-ai",
			"@dude1wudv/pi-coding-agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@dude1wudv/pi-tui": "/missing/path.ts",
		};
