...
class OmpLocal(BaseInstalledAgent):
        ...
        if self._binary:
            ...
            await self.exec_as_root(
                ...
                command=(
                    ...
                    "  echo 'ERROR: Alpine/musl base image; @dude1wudv/pi-natives ships no musl prebuilt' >&2; exit 3; "
    ...
    async def _install_local(self, environment: BaseEnvironment) -> str:
        ...
        await self.exec_as_agent(
            ...
            command=self._wrap(
                ...
                'echo "pinning native @dude1wudv/pi-natives-linux-$na@$ver"; '
                'bun add --production "@dude1wudv/pi-natives-linux-$na@$ver"'
            ),
            timeout_sec=900,
        )
    ...
    async def _install_published(self, environment: BaseEnvironment) -> str:
        app = f"{self._home}/.omp-bench/app"
        spec = f"@dude1wudv/pi-coding-agent@{self._pkg_version}"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
        ...
        return f"{app}/node_modules/@dude1wudv/pi-coding-agent/dist/cli.js"
