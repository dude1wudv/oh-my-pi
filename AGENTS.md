...
**Catalog import convention**: code in this repo imports catalog _values_ (bundled models, model-thinking helpers, identity, descriptors, model manager/cache) from `@dude1wudv/pi-catalog/<module>` — never via `@dude1wudv/pi-ai`. The pi-ai barrel re-exports only the model/effort _types_ its own signatures use (`Model`, `Api`, `ThinkingConfig`, `Effort`, …); type-only imports of those from `@dude1wudv/pi-ai` are fine.
...
- **Worker scripts**: workers re-enter the CLI entrypoint; never spawn separate worker entry modules. `cli.ts` declares itself as the worker host at startup (`declareWorkerHostEntry()` from `@dude1wudv/pi-utils/env`) and dispatches hidden argv selectors (`__omp_worker_stats_sync`, `__omp_worker_tab`, `__omp_worker_js_eval`, `__omp_worker_tiny_inference`) before loading the command registry. Spawn sites use:
  ```ts
  import { workerHostEntry } from "@dude1wudv/pi-utils";
...
| Binary lookup   | `$which("git")` from `@dude1wudv/pi-utils` | `spawnSync(["which", "git"])`      |
...
- **Tabs → spaces** via `replaceTabs()` (from `@dude1wudv/pi-tui` or `../tools/render-utils`).
