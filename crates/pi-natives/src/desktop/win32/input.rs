...
mod foreground {
	...
	impl ForegroundGuard {
		fn activate(id: &str) -> CoreResult<Self> {
			...
			// SAFETY: `target` was validated by `background::hwnd`; Win32 validates
			// whether it can become the foreground window.
			let activation_result = unsafe { SetForegroundWindow(target) };
			if previous != target && activation_result == 0 {
				return Err(DesktopError::input_failed(format!(
	...
	fn move_to(x: f64, y: f64) -> CoreResult<()> {
		...
		// SAFETY: GetSystemMetrics has no preconditions for these virtual-screen
		// metrics.
